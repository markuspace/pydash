...
A syntax preprocessor for Python. One em-dash `—` absorbs the boilerplate of LLM inference.
...
Flow: `hello.pyd` → `transpiler` → `hello.py` → output

### 1. hello.pyd (20 lines)
...
```bash
python transpiler.py hello.pyd > hello.py
python hello.py
```
